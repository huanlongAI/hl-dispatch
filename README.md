...
|----|------|------|------------|------|
| 裁决层 | 创始人 / 架构师 | 童正辉 | Owner (@tongzhenghui) | 产品决策、架构裁决、全部 Kotlin 代码生成（AI 协同）、PR 合并、里程碑签字 |
| 守护者 | Gate H 守护者 | 许久明 | Collaborator (@xujiuming) | 审计 AI 生成代码、架构合规、hl-framework 5 Starters + BOM、Gate H 签字 |
...
| 业务 | 产品经理 | 朱阳 | Collaborator (TBD) | 能力包 MVP 规格、Facts、reason_codes、验收用例、UAT 签收 |
| 基础设施 | 后端基础设施 | 魏鹏 | Collaborator (@wp159951) | starter-security JWT / starter-observability / HK Client SDK / 性能基线 |
| 核心组 | 技术验收官 | 李旭阳 | Collaborator (@LUXBYA) | 审计 AI 生成代码端到端正确性、Gate 3 签字、集成测试、回归测试（R-059） |
| 旧工程 | 旧工程交付 | 刘建成 | Collaborator (@jianchengl) | 旧工程交付管理、数据态打通协调（不纳入唤龙核心组，R-059） |
...
| 飞书群 | 用途 | 成员 |
|--------|------|------|
| #唤龙-工程通知 | PR/Issue/CI 事件自动推送 | 全团队 |
| #唤龙-任务协同 | 日常任务讨论主阵地 | 业务/工程/基础设施 |
| #唤龙-PM工作台 | PM 规格讨论专用 | PM + 相关 Reviewer |
| #唤龙-创始人指挥台 | 全局状态汇总、裁决通告 | Cowork + Gate 守护者 |
