...
有明确个人收件人的通知优先走飞书私聊，不默认刷群：

- `issues.assigned` 事件优先读取 `TEAM.yml`，按 GitHub handle 找到飞书 `open_id` 后通过 bot 私聊被指派人。
- 私聊发送需要仓库 secrets `FEISHU_BOT_APP_ID` 和 `FEISHU_BOT_APP_SECRET`，且飞书应用具备 bot 发消息权限。
- bot secrets 缺失、成员缺少 `open_id`、成员状态非活跃或私聊发送失败时，才 fallback 到原群 webhook。
- 本机手动私发使用 `scripts/feishu-direct-message.rb`，默认 dry-run；只有显式传 `--execute` 才会发送。
