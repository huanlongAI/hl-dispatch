...
跨仓库 Issue / PR 引用必须使用全限定 Markdown 链接，例如：

- `[huanlongAI/hl-platform#156](https://github.com/huanlongAI/hl-platform/pull/156)`
- `[huanlongAI/hl-dispatch#408](https://github.com/huanlongAI/hl-dispatch/issues/408)`

不要在回填正文中裸写 `#156`、`PR #156` 或 `Issue #156`；GitHub 会优先按当前仓库解析同编号对象，导致 AI 和成员读取到错误仓库的内容。
...
主线动作通知进入工程群，个人进展只走飞书私聊；普通评论、普通打开/关闭、普通指派不自动群发：

- `issues.labeled` 只有明确 action label 才进入 `AI native工程通知`：`action:decision_required` / `action:acceptance_ready` / `action:blocker` / `action:p0_failure` / `decision-request`。
- 手动 `workflow_dispatch` 可向 `AI native工程通知` 发送主线动作通知。
- 普通 `issue_comment.created` 不进入飞书群通知；确有动作项时使用 action label 或手动 `workflow_dispatch`。
- `issues.opened` / `issues.closed` 不再自动进入飞书群；需要通知时使用 action label 或手动 `workflow_dispatch`。
- `issues.assigned` 不由 `feishu-notify.yml` 自动触发；个人进展仍只允许私聊，不允许群 fallback。

- 私聊发送使用 `scripts/feishu-direct-message.rb`，脚本读取 `TEAM.yml`，按 GitHub handle 找到飞书 `open_id` 后通过 bot 私聊被指派人。
...
- bot secrets 缺失、成员缺少 `open_id`、成员状态非活跃或私聊发送失败时，只写 warning 并 fail-open，不发送群 fallback。
...
- Huanlong GitHub SSOT 下的 owner / DRI / verifier / evidence 私聊催办，必须先使用 `hl-owner-confirmation-dispatch` 做 route / dry-run 投影预检，再使用 `scripts/feishu-direct-message.rb`；不得直接裸调 `lark-cli im +messages-send`。
...
- `--idempotency-key` 可以使用任务侧长 key；helper 会在发送前归一化为飞书 `uuid` 可接受的稳定短 key。

示例：

```bash
# 先在 Codex 中触发 hl-owner-confirmation-dispatch，确认 route / dry-run 通过
...
在对应 run 的 summary 中查看 `Feishu delivery ledger`，或用日志里的 `FEISHU_DELIVERY_LEDGER_JSON` 单行 JSON 做命令行检查。台账不记录飞书 `open_id`、密钥或私聊会话全文；内容准确性以 GitHub Issue / PR / Actions run 的标题、标签、指派人和 URL 为准。
