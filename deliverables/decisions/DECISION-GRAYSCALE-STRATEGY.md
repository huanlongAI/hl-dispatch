...
**背景**：2026-03-04 对齐沟通会上，曾正龙团队提出灰度策略需要明确路由机制及后端功能归属
...
曾正龙团队指出：**灰度路由是后端功能，运维负责使用而非开发**。这个判断完全正确。
...
|--------|------|--------|--------|------|
| GrayscaleRoutingFilter 开发 | Gateway 层灰度路由 filter 实现 + 稳定哈希（Murmur3）+ 本地缓存 + 熔断 + 单元测试（含已知向量表） | 童正辉 + AI | S2 | TenantIdFilter（S1 已交付） |
| Nacos 灰度配置模板 | grayscale.yaml 配置模板 + config_version 机制 + 文档 | 童正辉 + AI | S2 | Nacos 集群（S0 曾正龙已部署） |
| FeatureToggleFilter 开发 `[v1.1 F1]` | Gateway 模块级功能开关 filter + feature-toggles.yaml 配置 + 单元测试 | 童正辉 + AI | S2 | GrayscaleRoutingFilter |
| S2 最小可用操作面 `[v1.1]` | 通过 Nacos 配置变更即可操作白名单/阈值/模块开关（无需 API），曾正龙获得 Nacos 灰度+功能开关配置命名空间写权限 | 童正辉（配置设计）+ 曾正龙（权限配置） | S2 | GrayscaleRoutingFilter + FeatureToggleFilter |
| 灰度操作 API | 查询/修改灰度比例的正式管理接口（封装 Nacos 操作，供运维调用） | 童正辉 + AI | S4（压测前） | — |
| 灰度操作 SOP（草案） `[v1.1]` | S3 出草案（基于 Nacos 直接操作），S5 升级为正式版（基于 API） | 曾正龙 | S3 草案 / S5 正式版 | S2 最小可用操作面 / S4 灰度 API |
| 灰度回滚演练 `[v1.1]` | 在 Staging 环境执行一次完整回滚演练，输出演练报告（≤5 分钟回滚验证） | 曾正龙 | S4（压测前） | SOP 草案 + Staging 环境 |
| 灰度切流执行 | 按 SOP 执行 1%→10%→50%→100% 切流 + 72h 观察（每档需满足扩量门禁） | 曾正龙 | S5 | 正式版 SOP + 扩量门禁表 |
| 灰度监控大盘 | 按 tenant_id 分组的请求量/错误率/p95 延迟/关键 reason_code 可视化（含扩量门禁指标） | 曾正龙 | S3~S4 | starter-observability（S1 魏鹏已交付） |

### 3.3 运维操作边界（回答曾正龙团队的问题）

明确划分"建功能"与"用功能"的边界：

| 类别 | 内容 | 负责人 |
|------|------|--------|
| **后端建设**（不是运维的活） | GrayscaleRoutingFilter 开发、灰度操作 API 开发、路由逻辑实现 | 童正辉 |
| **运维操作**（曾正龙的活） | 调用灰度 API 修改阈值、执行切流 SOP、监控观察、回滚执行 | 曾正龙 |
| **协作项** | 灰度监控大盘需要后端埋点（Micrometer）+ 运维配置 Grafana 面板 | 魏鹏（埋点）+ 曾正龙（面板） |
...
| 变更项 | 原内容 | 新增/修改内容 |
|--------|--------|-------------|
| S2 童正辉交付物 | HK 7 模块 PG 集成 + 审计链路 + Gateway ProtocolGate | **新增**：GrayscaleRoutingFilter + Nacos 灰度配置模板 |
| S4 童正辉交付物 | hl-console-native API 联调 | **新增**：灰度操作 API |
| S5 曾正龙交付物 | 终检报告 + 灰度切流 SOP + 72h 观察报告 | 不变（SOP 内容需要包含灰度 API 调用方式） |
| RACI 灰度切流行 | A=童正辉, R=曾正龙 | 不变（A 负责功能建设决策，R 负责切流执行） |
...
**扩量审批流程**：曾正龙出具指标报告 → 童正辉审批 → 曾正龙执行阈值变更。
...
| 视图 | 说明 | 实现方 |
|------|------|--------|
| 请求量分布 | 新/旧版本各接收多少请求，按租户分组 TOP-N | 曾正龙（Grafana） |
| 错误率对比 | 新版本租户 vs 旧版本租户的 5xx/4xx 比率 | 曾正龙（Grafana） |
| p95/p99 延迟对比 | 按版本分组的接口延迟百分位 | 曾正龙（Grafana） |
| 关键 reason_code 分布 | deny/error 类 reason_code 按租户和版本聚合 | 魏鹏（Micrometer 埋点）+ 曾正龙（面板） |
...
| 最小可用操作面 `[v1.1]` | 曾正龙可通过 Nacos 配置变更操作白名单/阈值/模块开关，无需代码部署 | S2 Gate H |
