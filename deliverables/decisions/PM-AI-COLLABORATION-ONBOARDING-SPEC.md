...
### Phase 1.5：飞书群邀请与协作规范学习（~10 分钟，创始人执行）
...
PM 加入飞书群后，**必须遵守**以下行为规范（详见 FEISHU-GITHUB-COLLABORATION-SPEC v1.0）：

| 规范 | 内容 | 截止时间 |
|------|------|---------|
| **讨论结论→GitHub 同步** | 飞书群的讨论达成结论后，PM 必须将结论、决策或行动项同步写入 GitHub（Issue Comment、PR Description、或决策文档） | 讨论后 **24 小时内** |
| **关联 GitHub 事项时附链接** | 在飞书群提及 GitHub Issue / PR 时，必须附加完整的 GitHub URL，便于追溯 SSOT | 即时 |
| **SSOT 优先级** | GitHub hl-dispatch/hl-contracts/hl-platform 为企业 SSOT；飞书为过程沟通辅助 | 持续遵守 |

**关键约束**：

- 飞书群用于**实时沟通与快速反馈**，不替代 GitHub 的正式决策记录
- PM 负责确保飞书讨论的结论完整同步到 GitHub；漏失同步视为 PM 工作不完整
- 跨仓库变更涉及的讨论，必须在 hl-dispatch（PM 协调枢纽）中留下决策记录
