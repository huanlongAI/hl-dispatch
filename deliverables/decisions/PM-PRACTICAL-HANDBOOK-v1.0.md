...
| 群 | PM 在这里做什么 |
|----|---------------|
| **PM 工作台通道** | 规格方向讨论、与创始人预沟通、收 Cap-Spec PR review 通知 |
| **任务协同通道** | 技术问题提问、跨角色协调、进度同步 |
| **工程通知通道** | 看 CI 状态（失败了赶紧修）、看 PR 合并通知 |
...
| 时段 | 飞书 | GitHub |
|------|------|--------|
| 上午 | PM 工作台通道：看 review 评论 | 处理 PR review feedback |
| | 工程通知通道：看 CI 状态 | 修复 CI 失败 |
| 下午 | PM 工作台通道：讨论规格方向 | AI 编码 → 提交 PR |
| | 任务协同通道：技术问题提问 | |
...
**想提裁决请求**：
1. 飞书 PM 工作台通道 简要说明 → 2. 创建 GitHub Issue → 3. 飞书说"已建 Issue #xxx" → 4. 等创始人在 GitHub 裁决
