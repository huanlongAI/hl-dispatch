...
**版本**：v1.2
**日期**：2026-03-17
**状态**：LOCKED（D-1~D-5 裁决完毕 + R-056/R-057/R-058 修订：无业务开发者、AI 全覆盖、核心组 7 人）
...
- 所有 Kotlin 代码 = 创始人 + AI 生成（含 contract / domain / adapter / DDL / P0-5 测试）
- 选择 Kotlin 的首要原因 = AI 生成的可靠性和长期维护优于 Java
...
② 契约注册            PM + 创始人   reason_codes.csv PR             Gate H
                                   ...
                                   PRD 文档集
                      │
③ 完整实现            创始人 + AI   contract/ 层接口                 CI 8 门禁
                                   domain 层逻辑 + adapter 层
                                   DDL + RLS + 五必过测试（P0-5）
                      │
④ 代码审计            Gate H       代码健康 + 架构合规               Gate H 签字
                      Gate 3       端到端业务正确性                   Gate 3 签字
                      │
⑤ 集成验证            技术验收官    端到端链路测试                    Gate 3
...
### 2.3.1 飞书协作通道（FEISHU-GITHUB-COLLABORATION-SPEC v1.0）
...
1. **GitHub 是事实真源**：所有正式决定、需求定义、代码变更均以 GitHub（Issue / PR / RULINGS.md）为绝对权威。
...
- ❌ 重大决定只在飞书记录（决定必须写入 RULINGS.md）
- ❌ 超过 7 天的讨论线程不转化为 Issue/PR（沉默即放弃）
...
**生产模型（R-057 修订）**：创始人 + AI 生成所有能力包的完整 Kotlin 实现。3-5 包按优先级逐包推进，Gate H / Gate 3 逐包审计。并行度受限于审计带宽而非开发人力。PM 按能力包分工定义需求。
...
| 操作 | PM | Gate H | Gate R | Infra | 验收官 | 创始人 |
|------|-----|--------|--------|-------|--------|--------|
| 提交 hl-contracts PR | ✅ | ✅ | — | — | — | ✅ |
| 审批 hl-contracts PR | — | ✅ | — | — | — | ✅ |
| 提交 hl-platform PR | — | ✅ | — | ✅ | — | ✅ |
...
| 业务开发者 / 后端开发岗 | R-056 修订：SAAC §1.3 逻辑终点——AI 生成全部 Kotlin 代码，不设人工开发岗 |
| 「骨架生成→人工填充」分工模式 | R-057 修订：创始人+AI 生成完整实现，无填充环节 |
