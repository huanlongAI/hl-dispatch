...
| **业务开发者** | 待定（R-053 扩编 2 人 + 后续视需求） | 能力包实现 | 基于 DEVPACK 模板开发能力包；domain 层 + adapter 层 Kotlin 实现；五必过测试编写 |
...
| 角色 | 人员 | 职责 |
|------|------|------|
| **项目协调** | 刘建成 | 进度跟踪、GitHub Issue/PR 流转协调、Sprint 看板维护、风险预警 |
| **技术验证** | 李旭阳 | 端到端验证、能力包集成测试、Gate 3（业务合规）执行 |

**说明**：刘建成和李旭阳在 hl-dispatch README 中已出现。他们不属于 R-053 核心组 6 人，但作为协作生态的参与者有明确价值。项目协调在全新构建中的定位从「管理 13 人适配团队」变为「维护 Sprint 看板 + 风险预警 + 跨角色协调」。
...
| 项目协调（刘建成） | Sprint 看板建立 + Issue 模板验证 |
...
| 项目协调（刘建成） | 能力包讨论协调；S3 demo 包选型会议组织 |
...
| 技术验证（李旭阳） | demo 包端到端链路验证 |

**S4 首批能力包（~4-6 周）**

| 角色 | 交付物 |
|------|--------|
| 创始人 + AI | 3-5 个核心 biz.* 骨架生成 |
| 业务开发者 | 能力包业务逻辑填充 + 测试（可并行多包） |
...
| **Gate 3** | 李旭阳 | 端到端业务合规 + 异常安全 | 能力包集成验证 |
...
| 操作 | PM | 业务开发者 | Gate H | Gate R | Infra | 创始人 |
|------|-----|-----------|--------|--------|-------|--------|
| 提交 hl-contracts PR | ✅ | ✅ | ✅ | — | — | ✅ |
| 审批 hl-contracts PR | — | — | ✅ | — | — | ✅ |
| 提交 hl-platform PR | — | ✅ | ✅ | — | ✅ | ✅ |
...
| 编号 | 事项 | 影响 | 建议时间 |
|------|------|------|---------|
| D-1 | 业务开发者扩编（R-053 核心组 +2 待定） | S3 demo 包人力 | S0 期间 |
| D-2 | S4 能力包并行开发时的人力模型 | 3-5 包同时开发需要多少业务开发者 | S2 中期 |
| D-3 | QA 角色定义（测试框架、仓库归属、与 P0-5 五必过的分工） | 测试覆盖 | S1 期间 |
| D-4 | 前端交付规范——hl-console-native（Swift）vs 消费者端（Flutter） | 前端协作效率 | S2 期间 |
| D-5 | 刘建成/李旭阳的正式角色定位（是否纳入核心组） | 组织清晰度 | S0 期间 |
