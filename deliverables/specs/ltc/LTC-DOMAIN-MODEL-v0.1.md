...
| 上下文 | 职责 | 不负责 |
|---|---|---|
| Endpoint Sentinel（端点哨兵） | 公司固定资产电脑安装、设备绑定、常驻运行、心跳、健康状态、断链检测。Windows 优先，Mac 第二平台。当前 Windows Endpoint Alpha 可先定义 service verification / installer manifest / evidence chain contract。 | 绩效裁决、内容检查、飞书写入、资产管理、在非 Windows 环境声明真实服务完成。 |
...
### 3.10 DahuiZiEvidenceReference（大辉子证据引用）

表示 LTC 可生成、可签名、可被大辉子引用的证据引用；不证明大辉子真实消费。
...
- 它是 dry-run evidence reference，不是大辉子已消费证据，也不是最终绩效结果。
...
### 3.10A WindowsAlphaVerificationEvidence（Windows Alpha 验证证据）

表示外部 Windows runner 提交的 sanitized observation。当前只用于实机化前置合约，不代表 LTC 自己执行了 Windows 服务命令。
...
- `runningStatus`
- `startupMode`
- `heartbeatSignatureState`
- `heartbeatSignatureHash`
- `servicePlanIdHash`
- `runnerIdHash`
- `timestampBucket`
- `observationHash`
...
- 非 Windows 环境不得声明真实 service registered、startup auto 或 process running 已验证。
- observation 不得包含 command output、PowerShell transcript、raw local path、raw service path、service binary path 或完整命令参数。
- 通过校验只表示 sanitized observation recorded，不表示 LTC 完成服务注册、安装或开机自启。

### 3.10B WindowsInstallerArtifactManifest（Windows 安装器清单）

表示 Windows installer artifact 的 hash-only manifest contract。当前不构建、不签名、不安装真实 MSI / installer。
...
- `companyAssetScope`
- `manifestHash`
- `installExecutionClaim`
- `windowsBuildVerification`

不变量：

- manifest 必须声明公司固定资产范围。
- manifest 不得包含 raw local path、operator ID 明文、signing key 或 PowerShell transcript。
- `installExecutionClaim` 固定为 `not_executed_by_ltc`，直到 Windows 构建和安装验证单独闭合。

### 3.10C EndpointEvidenceChainSummary（端点证据链摘要）

表示 heartbeat、service observation、admin action、owner amendment 和 evidence envelope 的 hash-only 链接摘要。
...
- 缺少签名 heartbeat 不能进入 healthy chain。
- 断链、恢复、管理员动作和员工说明 amendment 可以并存。
- 员工说明只能以 amendment hash 关联，不覆盖原 evidence。
- chain summary 不产生绩效结论，不输出员工说明明文或管理员理由明文。
...
| `EvidenceFeedDryRunPrepared` | Evidence Feed | Dahuizi |
| `DahuiZiEvidenceReferenceAvailable` | Evidence Feed | Compliance Audit |
...
CandidateObserved -> Sanitized -> Aggregated -> EvidenceHeld -> DryRunPlanPrepared
Sanitized -> HeldForReconfirmation -> DryRunPlanPrepared
```
