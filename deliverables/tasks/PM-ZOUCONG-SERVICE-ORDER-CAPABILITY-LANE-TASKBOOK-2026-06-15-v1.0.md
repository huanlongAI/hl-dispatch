...
本任务书不启动产品代码实现，不授权工程理解通过、production runtime（生产运行态）、active contract（生效契约）、active registry（生效注册）、真实用户数据、真实客户资产变更、真实服务履约、支付 / 退款 / 结算、secrets、deploy 或 release。

## 术语说明

- `PM readiness pack`：产品经理准备包。用于说明能力边界、首个受限切片、验收标准和未决问题。
- `工程理解确认` / `Implementation Readiness`：工程师在当前 GitHub 入口内回复的轻量业务理解确认，旧 HPRD 模板仅作检查清单参考；它不是第二份 PRD，也不是技术设计。
- `PM understanding pass`：PM 确认工程师业务理解忠于能力规格。它只允许在本任务书签定范围内进入受限工程实现，不等于生产授权。
...
目标是建立一条清晰交接链路：PM 先确认业务语义和首个受限切片；Founder / Gate 再命名工程 owner；工程 owner 在当前唯一入口内提交工程理解确认；PM 审理解确认通过后，才可在本任务书边界内进入受限实现。
...
-> Founder / Gate 命名工程 owner 和首个受限切片
-> 工程 owner 在当前入口提交工程理解确认
-> 邹骢按 Cap-Spec 审工程理解确认
-> PM understanding pass 后进入本任务书内的受限工程实现
...
PM understanding pass 只表示工程师读懂了业务语义，并可在本任务书签定范围内做受限实现。它不授权生产运行态、生效契约注册、真实用户数据、支付 / 计费 / 退款 / 结算、secrets、deploy 或 release。
...
pm_capability_readiness_pack:
  ...
  conclusion: ready_for_engineer_understanding | ready_for_engineer_hprd | needs_founder_gate_decision | blocked
  ...
  engineering_understanding_review_standard:
    - "<工程理解确认必须解释正确的业务语义>"
  not_authorized:
    - production_runtime
    - active_contract
...
工程理解确认只能在以下条件同时满足后启动：

- 本任务书已作为 GitHub SSOT 发布。
- 邹骢已提交 `ready_for_engineer_understanding`；历史回复里的 `ready_for_engineer_hprd` 仅作为同义兼容，不要求新开 HPRD 入口。
- Founder / Gate 已命名工程 owner 和第一个受限工程切片。

工程 owner 必须在当前唯一 GitHub 入口内回复工程理解确认，不新开 HPRD Issue，不新开 taskbook，不新增独立 HPRD 文档。确认内容必须说明：
...
邹骢只在同一入口内回复：`通过`、`需修正` 或 `需 Founder/Gate 裁决`。

## 8. 人类可读门禁

每个 PM 回复、工程理解确认、PR、`gap_report`、Gate 报告和验收包都必须让 Founder、PM 和工程师能直接读懂：
