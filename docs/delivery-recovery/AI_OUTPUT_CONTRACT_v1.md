...
The live GitHub language gate rejects `ai-output:v1` comments that contain the canonical black-box phrases above only after the shared `sentinel-shared` gate update is merged into the reusable workflow consumed by this repository. If the work needs more context, use `gap_report` and name the missing context, evidence, authorization, or owner.
